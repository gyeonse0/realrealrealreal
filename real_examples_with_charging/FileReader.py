...
class FileReader:
    ...
    def randomize_vrp_data(self, data, time=None, max_value=None, key_seed=None):
        # 키 선택을 위한 랜덤 시드 설정
        ...
        if time is not None:
            while True:
                new_values = np.random.choice(time_slots, size=values_except_depot.shape, p= [0.6, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05])
