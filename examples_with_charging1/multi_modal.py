...
repair_operators = [Rep.drone_first_truck_second, Rep.truck_first_drone_second, Rep.heavy_truck_repair]
...
while current_num <= iteration_num:
    if current_num==0:
        ...
        destroyed_state = selected_destroy_operators(init, rnd_state)
        ...
        if MultiModalState(current_states[-1]).cost_objective() > MultiModalState(k_opt_state).cost_objective():
...
print("\nIteration #:",min_index)
pct_diff = 100 * (MultiModalState(current_states[min_index]).cost_objective() - init.cost_objective()) / init.cost_objective()
print(f"\nThis is {-(pct_diff):.1f}% better than the initial solution, which is {init.cost_objective()}.\n")
