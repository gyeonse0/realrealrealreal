...
class MultiModalState:
        ...
        for route in self.routes:
            ...
            for customer in route[1:]: 
                if data["priority_delivery_time"][customer[0]] != 0:
                    late_time += (combined_time[customer[0]] - data["priority_delivery_time"][customer[0]])
                    
                    if late_time > 0:
                        if refund_cost > data["price"][customer[0]]:
                            refund_cost += data["price"][customer[0]]
                        else:
                            refund_cost += np.exp(late_time / 10)
    ...
    # def latetime(self, time, customer):
    #     if data["priority_delivery_time"][customer[0]] != 0:
    #         latetime = (time[customer] - data["priority_delivery_time"][customer])
    #         if latetime > 0:
